...
public class DisplayListener implements Listener {
    ...
    private final ArrayList<ItemStack> allServerRecipeResults = new ArrayList<>();
    ...
    public void onShulkerBoxClose(InventoryCloseEvent event) {
        if (event.getInventory().getHolder() instanceof ShulkerBox) {
            ...
            AbstractShop shop = plugin.getShopHandler().getShopByChest(box.getBlock());
            if (shop != null) {
                new BukkitRunnable() {
                    @Override
                    public void run() {
    ...
    public void onItemHook(PlayerFishEvent event) {
        if (event.getCaught() != null && Display.isDisplay(event.getCaught())) {
            event.setCancelled(true);
        }
    }
