        ...
        if (args.length == 0) {
            ...
            if (args[0].equalsIgnoreCase("list")) {
                ...
                if (sender instanceof Player) {
                    ...
                    if ((plugin.usePerms() && player.hasPermission("shop.operator")) || player.isOp()) {
                        ...
                        if (plugin.useVault()) {
                            ...
                            player.sendMessage(ChatColor.GRAY + "The server is using " + ItemNameUtil.getName(plugin.getItemCurrency()) + " as currency.");
                            player.sendMessage(ChatColor.GRAY + "To change this run the command '/shop setcurrency' with the item you want in your hand.");
                        }
                        return true;
                    }
                } else {
                    sender.sendMessage("The server is using " + ItemNameUtil.getName(plugin.getItemCurrency()) + " as currency.");
                }
                ...
                if (sender instanceof Player) {
                    ...
                    if ((plugin.usePerms() && player.hasPermission("shop.operator")) || player.isOp()) {
                        ...
                        if (plugin.useVault()) {
                            ...
                            player.sendMessage(ChatColor.GRAY + "The server is now using " + ItemNameUtil.getName(plugin.getItemCurrency()) + " as currency.");
                        }
                        return true;
                    }
                } else {
                ...
                if (sender instanceof Player) {
                    ...
                    ItemStack handItem = player.getInventory().getItemInMainHand();
                    if (handItem.getType() != Material.AIR) {
                        plugin.setGambleDisplayItem(handItem);
                    } else {
