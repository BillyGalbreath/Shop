...
public class EnderChestHandler {
    ...
    private final Map<UUID, Inventory> enderChestInventories = new HashMap<>(); //for use with ender chest shops while offline
    private final List<UUID> playersSavingInventories = new ArrayList<>();
    ...
    private void loadEnderChests() {
        ...
        File[] filesList = enderDirectory.listFiles();
        if (filesList == null) {
            return;
        }
